...
For example, we observe that cluster `r chosen.text` has the highest average expression for the triacylglycerol metabolic process GO term (Figure \@ref(fig:lipid-synth-violin)), consistent with the proposed identity of those cells.

```{r lipid-synth-violin, fig.cap="Distribution of average log-normalized expression for genes involved in triacylglycerol metabolic process, for all cells in each cluster of the mammary gland dataset."}
plotColData(sce.mam, y=I(aggregated["GO:0006641",]), x="label")
```

```{r, echo=FALSE}
maxed <- vapply(split(aggregated["GO:0006641",], colLabels(sce.mam)), median, 0)
...
```{r, echo=FALSE}
maxed <- vapply(split(logcounts(sce.mam)["Dbi",], colLabels(sce.mam)), median, 0)
stopifnot(identical(names(maxed)[which.max(maxed)], '2'))
```
